...
try:
    ...
    pd = None  # keep OPLEM importable,fallback if panda is not avalaible


class PVAsset(NondispatchableAsset):
    """
    PV model used in this IEP project.

    Takes irradiance and temperature as inputs and returns power output.
    If temperature is not provided, 25°C is assumed.
...
class WTAsset(NondispatchableAsset):
    """
    Wind turbine model using a simple power curve.

    Includes cut-in, rated, and cut-out behaviour.
    ...
    def __init__(
        ...
        # validate physics/limits before building generatin profiles
        self._validate_params()
