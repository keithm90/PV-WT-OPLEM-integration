...
try:
    ...
    pd = None  # keep OPLEM importable even if pandas isn't installed


class PVAsset(NondispatchableAsset):
    """
    Photovoltaic generator asset.

    Inputs to generate():
      - ghi_wm2 : Global Horizontal Irradiance [W/m^2] (Series/array/list)
      - temp_c  : Ambient temperature [°C] (Series/array/list). If None -> assumes 25°C.

    Simple semi-empirical model:
      P = P_rated * (GHI/1000) * (1 + gamma*(T-25)) * PR
      clipped to [0, P_rated*PR]
    """
    ...
    def __init__(
        ...
        pr: float = 1.0,          # performance ratio / derate factor (0–1)
        curt: bool = False,
    ):
        self.rated_power_kw = float(rated_power_kw)
        self.gamma_per_c = float(gamma_per_c)
        ...
        # Pre-allocate arrays (so OPLEM always has consistent fields)
        Pnet = np.zeros(T, dtype=float)
        Qnet = np.zeros(T, dtype=float)

        # Robust init: works whether NondispatchableAsset expects Pnet/Qnet or not
        try:
            super().__init__(
                Pnet=Pnet,
                Qnet=Qnet,
            ...
            # if base class didn’t create them, set them here
            self.Pnet = Pnet
            self.Qnet = Qnet

        # EMS buffers (safe even if base class already has them)
        if not hasattr(self, "Pnet_ems"):
            self.Pnet_ems = np.zeros(T_ems, dtype=float)
        if not hasattr(self, "Qnet_ems"):
            self.Qnet_ems = np.zeros(T_ems, dtype=float)

    def generate(self, ghi_wm2, temp_c=None):
        """
        Returns PV active power profile (kW) of length T.
        """
        if ghi_wm2 is None:
            raise ValueError("ghi_wm2 must be provided")

        ghi = np.asarray(ghi_wm2, dtype=float).reshape(-1)
        ghi = np.clip(ghi, 0.0, None)

        if len(ghi) != self.T:
            raise ValueError(f"GHI length {len(ghi)} must equal T={self.T}")

        if temp_c is None:
            temp = np.full(self.T, 25.0, dtype=float)
        else:
            temp = np.asarray(temp_c, dtype=float).reshape(-1)
            if len(temp) != self.T:
                raise ValueError(f"Temp length {len(temp)} must equal T={self.T}")

        # model (kW)
        p_kw = self.rated_power_kw * (ghi / 1000.0) * (1.0 + self.gamma_per_c * (temp - 25.0))
        p_kw = p_kw * self.pr

        # physical limits
        p_kw = np.clip(p_kw, 0.0, self.rated_power_kw * self.pr)

        q_kw = self._q_from_pf(p_kw, self.power_factor)

        # store internally (keep same arrays)
        self.Pnet[:] = p_kw
        self.Qnet[:] = q_kw

        return self.Pnet.copy()
    ...
    def _q_from_pf(self, p_kw, pf):
        p_kw = np.asarray(p_kw, dtype=float)
        if pf >= 1.0:
            return np.zeros_like(p_kw)
        return p_kw * np.tan(np.arccos(pf))

class WTAsset(Asset):
    """
    Wind turbine generator asset using a simple piecewise power curve.

    Expected weather input:
      - wind_ms : wind speed [m/s] (can be pandas Series, numpy array, or list)

    Piecewise model:
      P = 0                          v < v_cut_in
      cubic ramp                     v_cut_in <= v < v_rated
      P = P_rated                    v_rated <= v <= v_cut_out
      P = 0                          v > v_cut_out
    """
    ...
    def __init__(
        ...
        # --- store scalar params ---
        self.rated_power_kw = float(rated_power_kw)
        self.v_cut_in = float(v_cut_in)
        self.v_rated = float(v_rated)
        self.v_cut_out = float(v_cut_out)
        self.power_factor = float(power_factor)
        self.curt = bool(curt)
        self.eff_wt = float(eff_wt)

        # validate physics/limits before building profiles
        self._validate_params()

        # --- base Asset init (do NOT pass Pnet/Qnet here) ---
        super().__init__(
            bus_id=bus_id,
            dt=dt,
            T=T,
        ...
        # --- initialise time-series buffers (hourly + EMS) ---
        self.Pnet = np.zeros(self.T, dtype=float)
        self.Qnet = np.zeros(self.T, dtype=float)

        self.Pnet_ems = np.zeros(self.T_ems, dtype=float)
        self.Qnet_ems = np.zeros(self.T_ems, dtype=float)

    def generate(self, wind_ms):
        """
        Generate wind active power profile (kW) from wind speed (m/s).
        Accepts pandas Series / numpy array / list.
        Returns: numpy array of length T (kW).
        """
        v = np.asarray(wind_ms, dtype=float).reshape(-1)
        v = np.clip(v, 0.0, None)

        if len(v) != self.T:
            raise ValueError(f"wind_ms length {len(v)} must equal T={self.T}")

        p = np.zeros_like(v, dtype=float)

        # cubic ramp region: v_cut_in <= v < v_rated
        ramp = (v >= self.v_cut_in) & (v < self.v_rated)

        #v_ramp = 17
        #print(f"WTAsset generate() using v_ramp = {v_ramp}")

        denom = (self.v_rated**3 - self.v_cut_in**3)

        if denom > 0:
            p[ramp] = self.rated_power_kw * ((v[ramp]**3 - self.v_cut_in**3) / denom)

        # rated region: v_rated <= v <= v_cut_out
        rated = (v >= self.v_rated) & (v <= self.v_cut_out)
        p[rated] = self.eff_wt * self.rated_power_kw # Accounting 4% losses

        # enforce bounds
        p = np.clip(p, 0.0, self.rated_power_kw)
        p[v > self.v_cut_out] = 0.0                     #No generation after cutout v
        # reactive power from power factor
        q = self._q_from_pf(p, self.power_factor)
    ...
    def _validate_params(self):
        ...
        if not (0.0 < self.power_factor <= 1.0):
            raise ValueError("power_factor must be in (0, 1]")
        if not (0 < self.v_cut_in < self.v_rated < self.v_cut_out):
            raise ValueError("Need 0 < v_cut_in < v_rated < v_cut_out")
