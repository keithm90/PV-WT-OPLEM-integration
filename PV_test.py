...
df = pd.read_csv(PV_FILE, skiprows=3)
...
if df["time"].isna().any():
    bad = df.loc[df["time"].isna(), "time"].head(5)
    raise ValueError(f"Some time values could not be parsed. Examples:\n{bad}")

df = df.set_index("time")

# GHI approximation (W/m^2)
df["ghi_wm2"] = df["irr_dir"].astype(float) + df["irr_dif"].astype(float)

# If it’s accidentally in kW/m^2, convert to W/m^2
...
# -------------------------
# 2) Build PVAsset + generate model
# -------------------------
T = len(df)
dt = 1.0
dt_ems = 1.0
...
# Use PR here (so the model is consistent)
PR = 0.79
...
model_kw = pv.generate(ghi, temp)
ninja_kw = df["electricity_kw"].astype(float).to_numpy()
...
model_kw = np.asarray(model_kw[mask], dtype=float)
ninja_kw = np.asarray(ninja_kw[mask], dtype=float)
...
energy_err_pct = 100.0 * (E_model_kwh - E_ninja_kwh) / E_ninja_kwh if E_ninja_kwh != 0 else np.nan
...
plt.plot(df_valid.index[:samples], df_valid["ninja_kw"].to_numpy()[:samples], label="Ninja ref (kW)")
plt.plot(df_valid.index[:samples], df_valid["model_kw"].to_numpy()[:samples], label="Model (kW)")
plt.xlabel("Time")
plt.ylabel("Power (kW)")
